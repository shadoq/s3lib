...
public class Gradient implements ProceduralInterface, FilterPixmapInterface{
	 ...
	 */
	public static void generate(final Pixmap pixmap, final Color topLeft, final Color topRight, final Color bottomLeft, final Color bottomRight,
								final float alpha){
		...
		if (DEBUG){
			Gdx.app.log("Gradient::generate()",
						"pixmap: " + pixmap + " topLeft: " + topLeft + " topRight: " + topRight + " bottomLeft: " + bottomLeft + " bottomRight: " + bottomRight + " alpha: " + alpha);
		}
		...
		for (int y = 0; y < height; y++){
			for (int x = 0; x < width; x++){
				...
				r =
				(int) (r + ((topLeft.r * tLeft * 255) + (topRight.r * tRight * 255) + (bottomLeft.r * lBottom * 255) + (bottomRight.r * rBottom * 255) * alpha));
				g =
				(int) (g + ((topLeft.g * tLeft * 255) + (topRight.g * tRight * 255) + (bottomLeft.g * lBottom * 255) + (bottomRight.g * rBottom * 255) * alpha));
				b =
				(int) (b + ((topLeft.b * tLeft * 255) + (topRight.b * tRight * 255) + (bottomLeft.b * lBottom * 255) + (bottomRight.b * rBottom * 255) * alpha));
